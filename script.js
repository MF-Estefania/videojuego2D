...
const clickSound = new Audio('img_assets/click-sound.mp3');
const startSound = new Audio('img_assets/start-sound.mp3');
      ...
      if (this.opacity <= 0) {
        ...
        if (index > -1) {
          ...
          if (ghosts.length < maxGhosts) {
            ghosts.push(new Ghost(Math.random() * canvas.width, Math.random() * canvas.height, 'img_assets/fantasma.png'));
          }
        }
      }
    }
...
function createGhosts() {
  ...
  for (let i = 0; i < maxGhosts; i++) {
    ...
    ghosts.push(new Ghost(x, y, 'img_assets/fantasma.png'));
  }
}
